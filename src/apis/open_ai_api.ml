...
let create_file ~file ~purpose =
  ...
  let body =
    Request.add_form_encoded_body_param body "file" (fun x -> x) file
  in
  let body =
...
let create_transcription ~file ~model ?prompt ?(response_format = "json")
    ?(temperature = 0.) ?language () =
  let open Lwt in
  ...
  in
  let body =
    Request.add_form_encoded_body_param body "model" (fun x -> x) model
  in
  let body =
    Request.maybe_add_form_encoded_body_param body "prompt" (fun x -> x) prompt
  in
  let body =
    Request.add_form_encoded_body_param body "response_format"
      (fun x -> x)
      response_format
  in
  let body =
    Request.add_form_encoded_body_param body "temperature" string_of_float
      temperature
  in
  let body =
    Request.maybe_add_form_encoded_body_param body "language"
      (fun x -> x)
      language
  in
  let body = Request.finalize_form_encoded_body body in
...
let create_translation ~file ~model ?prompt ?(response_format = "json")
    ?(temperature = 0.) () =
  let open Lwt in
