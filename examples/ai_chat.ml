...
let rec loop term history state =
  ...
  | Some input ->
      let message =
        Openai.Chat_completion_request_message.make
          ~role:`User
      ...
      let req =
        Openai.Create_chat_completion_request.make
          ~model:"gpt-3.5-turbo"
          ~messages:state
          ()
      in
      ...
      in
      let state =
        state @ [{role= message'.role; content= message'.content; name= None}]
      in
          ...
          let rec aux = function
            ...
            | line :: lines ->
                if String.length line > text_width then
                  ...
                  let line'' = String.sub line text_width (String.length line - text_width) in
                  line' :: aux (line'' :: lines)
                else line :: aux lines
          in
...
let chatgpt initial_state =
  ...
  Lwt.catch
    ...
    (function LTerm_read_line.Interrupt -> Lwt.return () | exn -> Lwt.fail exn)

let _ =
  let system_prompt =
    Openai.Chat_completion_request_message.make
      ~role:`System
      ~content:"You are a general-purpose assistant who provides detailed, helpful responses."
      ()
  in
  let init_message =
    Openai.Chat_completion_request_message.make
